...
-- BRDs table (NEW) - Store generated BRDs per project
CREATE TABLE IF NOT EXISTS brds (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    project_id UUID NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
    title TEXT NOT NULL DEFAULT 'Business Requirements Document',
    content TEXT NOT NULL,
    markdown_content TEXT NOT NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Documents table (UPDATED - add project_id)
...
-- Conversations table (UPDATED - add project_id)
...
CREATE INDEX IF NOT EXISTS idx_brds_project_id ON brds(project_id);
...
-- Function to search similar chunks using vector similarity (UPDATED - filter by project)
...
DROP TRIGGER IF EXISTS update_project_on_document ON documents;
CREATE TRIGGER update_project_on_document
AFTER INSERT ON documents
FOR EACH ROW
EXECUTE FUNCTION update_project_timestamp();

DROP TRIGGER IF EXISTS update_project_on_conversation ON conversations;
CREATE TRIGGER update_project_on_conversation
AFTER INSERT ON conversations
FOR EACH ROW
EXECUTE FUNCTION update_project_timestamp();

DROP TRIGGER IF EXISTS update_project_on_brd ON brds;
CREATE TRIGGER update_project_on_brd
AFTER INSERT ON brds
FOR EACH ROW
EXECUTE FUNCTION update_project_timestamp();
...
CREATE POLICY "Allow all operations on brds" ON brds FOR ALL USING (true) WITH CHECK (true);
...
VALUES ('00000000-0000-0000-0000-000000000001'::uuid, 'Default Project', 'Initial project for existing data')
